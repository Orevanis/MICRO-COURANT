...
export const contractCallRateLimiter = rateLimit({
  ...
  handler: (req, res) => {
    logger.warn(
      `Contract call rate limit exceeded for: ${req.user?.stellarAddress || req.ip}, Path: ${req.path}`,
    );
    res.status(429).json({
      error: "Too many contract calls",
      message:
        "Rate limit exceeded for contract calls. Please wait before making another call.",
      retryAfter: 60,
    });
  },
  skip: (req) => {
    ...
    return (
      ...
      !req.path.startsWith("/api/settlement")
    );
  },
});
...
export const writeOperationRateLimiter = rateLimit({
  ...
  handler: (req, res) => {
    logger.warn(
      `Write operation rate limit exceeded for: ${req.user?.stellarAddress || req.ip}, Path: ${req.path}`,
    );
    res.status(429).json({
      error: "Too many write operations",
      message:
        "Rate limit exceeded for write operations. Please wait before making another write.",
      retryAfter: 30,
    });
  },
  skip: (req) => {
