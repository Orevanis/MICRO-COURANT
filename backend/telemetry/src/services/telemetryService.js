...
export class TelemetryService {
  ...
  async processReading(reading) {
    ...
    try {
      ...
      await this.updateHouseholdBalance(reading.household_id, reading.consumption_kwh);
      ...
      logger.info(`Reading processed: ${readingId} for meter ${reading.meter_id}`);
  ...
  async updateHouseholdBalance(householdId, consumptionKwh) {
    ...
    if (newBalance < 1000) { // Less than 1000 stroops
      await this.redis.publish(
        `alerts:${householdId}`,
        JSON.stringify({
          type: 'low_balance',
