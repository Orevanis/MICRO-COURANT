...
export class MeterSimulator {
  ...
  generateSignature(meterId, consumption) {
    ...
    return Buffer.from(`${meterId}-${consumption}-${Date.now()}`).toString('base64').substring(0, 32);
  }
  ...
  publishReading(reading) {
    ...
    this.client.publish(topic, JSON.stringify(reading), (err) => {
      if (err) {
        ...
        logger.debug(`Published reading for ${reading.meter_id}: ${reading.consumption_kwh} kWh`);
      }
    });
  }
  ...
  startSimulation(meterCount, interval) {
    ...
    logger.info(`Started simulation with ${meterCount} meters, ${interval}ms interval`);
  }
