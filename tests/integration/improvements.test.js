...
import { CircuitBreaker, CircuitBreakerManager, CircuitState } from '../../shared/circuit-breaker/src/index.js';
...
describe('Integration Tests - Adaptive Fraud Detection', () => {
  ...
  it('should adapt thresholds based on meter profile', async () => {
    ...
    const retrievedProfile = JSON.parse(await mockRedis.get('fraud:profile:meter-001'));
    ...
    const adaptiveThreshold = Math.max(2.0, 2.0 + (profile.standardDeviation / profile.averageConsumption));
  ...
  it('should update meter profile with new readings', async () => {
    ...
    await mockRedis.set('fraud:profile:meter-002', JSON.stringify(initialProfile));
    ...
    profile.averageConsumption = oldAverage + (newConsumption - oldAverage) / profile.readingCount;

    await mockRedis.set('fraud:profile:meter-002', JSON.stringify(profile));

    const updatedProfile = JSON.parse(await mockRedis.get('fraud:profile:meter-002'));
